...
var (
	tracerEvent = otel.Tracer("event timetable")
)
...
func HandleCreateEventTimetable(db *pgxpool.Pool) http.Handler {
	return http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			reqCtx := r.Context()
			ctx, span := tracerEvent.Start(reqCtx, "create eventt timetable")
			defer span.End()

			timetable := reqCtx.Value("event timetable").(EventTimetable)
